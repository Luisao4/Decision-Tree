...
class DecisionTree {
//Pertinent info about Tree Structure
	 ...
	 **/
//Inner class TreeNode. Saves data and has methods relevant to the decision tree, such as values, children, and printing that information
	class TreeNode {
...
//Attribute numbers, learnt tree (post training phase), unique variable strings and their quantity, number of training examples
	private String used;
...
//PrintTree. Does what it says on the tin
...
//Error message printing auxiliary method
	/** Print error message and exit. **/
	static void error(String msg) {
		System.err.println("Error: " + msg);
		System.exit(1);
	} // error()
//Auxiliary logarithm function
	static final double LOG2 = Math.log(2.0);
	
	static double xlogx(double x) {
		return x == 0? 0: x * Math.log(x) / LOG2;
	} // xlogx()
//Classify. Classifies the test data after training, via traverse method
...
//Traverse. Traverses the tree, through each branch, and finally reaching the leaf node
...
//train. Takes training data matrix, uses indexStrings method, and sends it to buildtree
...
//CheckUsedAttributes, as the name implies, checks what attributes have been used and replaced with "used" string.
	/**
...
//Creates subset out of the currentDataSet that is given	
...
//Calculates dataset entropy, assuming last column represents class	
	/**
...
//IndexStrings. Given 2DArray of training data, number each unique value per attribute(Attribute 2's 1st value is Strings[2][1], it's 2nd Strings[2][2], etc, and the number of different values is stringCount[2])
...
//PrintStrings Debugger (Print list of attribute values for each attribute, as well as index values)
...
//Parse CSV file
	static String[][] parseCSV(String fileName) throws FileNotFoundException, IOException {
        ...
        String[] testing = new String[lines];
		...
		for (int n = 0; n < lines; n++){
			...
			for (int f = 0; f < fields; f++){
                if(f==0){
                    sc.next();
                    //System.out.println(sc.next()); Remove ID da lista, não é requerido para o treino
                }
				if (sc.hasNext()){
					data[n][f] = sc.next();
                    //System.out.println(data[n][f]); Teste de dados recebidos
                }
				else{
